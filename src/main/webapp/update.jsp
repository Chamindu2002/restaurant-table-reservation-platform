...
<%--                           min="1"--%>
<%--                           max="50"--%>
<%--                           value="<%= request.getParameter("table") %>"--%>
<%--                           required>--%>
...
<%--                        <option--%>
<%--                                <%= "Standard".equals(request.getParameter("type")) ? "selected" : "" %>>--%>
<%--                            Standard--%>
<%--                        </option>--%>

<%--                        <option--%>
<%--                                <%= "VIP".equals(request.getParameter("type")) ? "selected" : "" %>>--%>
<%--                            VIP--%>
<%--                        </option>--%>
...
<head>
    ...
    <title>Edit Reservation — Réservation</title>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        :root {
            ...
            --input-bg: rgba(255,255,255,0.04); --input-border: rgba(201,168,76,0.2);
        }
        html, body { min-height: 100%; background: var(--ink); color: var(--cream); font-family: 'DM Sans', sans-serif; }
        body::before {
        ...
        .page { position: relative; z-index: 1; min-height: 100vh;
            display: flex; align-items: flex-start; justify-content: center; padding: 80px 24px 60px; }
        ...
        h1 { font-family: 'Cormorant Garamond', serif; font-size: 48px; font-weight: 300; line-height: 1.1; }
        h1 em { font-style: italic; color: var(--gold2); }

        /* ID badge at top */
        .id-badge {
            display: inline-flex; align-items: center; gap: 10px; margin-bottom: 32px;
            padding: 10px 18px; border: 1px solid var(--line); background: rgba(201,168,76,0.04);
        }
        .id-badge-label { font-size: 10px; letter-spacing: 0.2em; text-transform: uppercase; color: var(--muted); }
        .id-badge-val { font-family: 'Cormorant Garamond', serif; font-size: 20px; color: var(--gold2); }

        .panel { background: rgba(255,255,255,0.03); border: 1px solid var(--line); padding: 40px; }
        .section-title { font-size: 10px; letter-spacing: 0.3em; text-transform: uppercase;
            color: var(--gold); margin-bottom: 20px; padding-bottom: 8px; border-bottom: 1px solid var(--line); }
        ...
        .field input, .field select {
            background: var(--input-bg); border: 1px solid var(--input-border); color: var(--cream);
        ...
        .field input:focus, .field select:focus { border-color: var(--gold); background: rgba(201,168,76,0.05); }
        ...
        .type-toggle label {
            ...
            font-weight: 500; color: var(--muted); background: var(--ink); transition: all 0.2s;
        }
        .type-toggle input:checked + label { background: var(--gold); color: var(--ink); font-weight: 500; }
        .cost-preview { margin-top: 24px; padding: 20px 24px;
            background: rgba(201,168,76,0.06); border: 1px solid var(--line);
            display: flex; align-items: center; justify-content: space-between; }
        .cost-label { font-size: 11px; letter-spacing: 0.2em; text-transform: uppercase; color: var(--muted); }
        .cost-value { font-family: 'Cormorant Garamond', serif; font-size: 28px; font-weight: 300; color: var(--gold2); }
        .divider { height: 1px; background: var(--line); margin: 32px 0; }
        .actions { display: flex; gap: 16px; align-items: center; flex-wrap: wrap; }
        .btn-primary {
            padding: 14px 36px; background: #c9a84c; color: var(--ink); border: none;
        ...
        .btn-ghost {
            ...
            letter-spacing: 0.15em; text-transform: uppercase; text-decoration: none; transition: all 0.2s;
        }
        .btn-ghost:hover { border-color: var(--gold); color: var(--cream); }
        @media (max-width: 580px) { .row { grid-template-columns: 1fr; } h1 { font-size: 36px; } }
    </style>
</head>
<body>
...
<div class="page">
    <div class="container">
        ...
        <div class="breadcrumb">
            ...
            <span>›</span>
            <a href="reservation?action=viewAll">Reservations</a>
            <span>›</span>
            <span>Edit</span>
        </div>
        ...
        <div class="heading-block">
            ...
            <h1>Edit <em>Reservation</em></h1>
        </div>

        <div class="id-badge">
            <span class="id-badge-label">Editing ID</span>
            <span class="id-badge-val"><%= request.getParameter("id") %></span>
        </div>
            ...
            <form action="reservation" method="post">
                ...
                <div class="row">
                    <div class="field">
                        ...
                        <input type="text" value="<%= request.getParameter("id") %>" readonly>
                        <small>ID cannot be changed</small>
                    </div>
                    <div class="field">
                ...
                <div class="row">
                    ...
                    <div class="field">
                        ...
                        <input type="time" id="time" name="time"
                               value="<%= request.getParameter("time") %>" required>
                ...
                <div class="row">
                    <div class="field">
                        ...
                        <input type="number" id="guests" name="guests"
                               min="1" max="20" value="<%= request.getParameter("guests") %>" required>
                        <small>Maximum 20 guests per reservation</small>
                    </div>
                    <div class="field">
                        <label for="table">Table Number</label>
                        <input type="number" id="table" name="table"
                               min="1" max="50" value="<%= request.getParameter("table") %>" required>
                ...
                <div class="row single">
                    <div class="field">
                        <div class="type-toggle">
                            <input type="radio" name="type" id="typeStandard" value="Standard"
                                <%= "Standard".equals(request.getParameter("type")) ? "checked" : "" %>>
                            <label for="typeStandard">Standard — Rs. 1,000 / guest</label>
                            <input type="radio" name="type" id="typeVip" value="VIP"
                                <%= "VIP".equals(request.getParameter("type")) ? "checked" : "" %>>
                ...
                <div class="cost-preview">
                    <span class="cost-label">Updated Cost</span>
                ...
                <div class="actions">
                    ...
                    <a href="reservation?action=viewAll" class="btn-ghost">Cancel</a>
                </div>
            </form>
        </div>
...
<script>
    ...
    function updateCost() {
        ...
        costDisplay.textContent = 'Rs. ' + (guests * rate).toLocaleString('en-LK', {minimumFractionDigits: 2});
    }

    guestsInput.addEventListener('input', updateCost);
    radios.forEach(r => r.addEventListener('change', updateCost));
    updateCost(); // run on load with existing values
</script>
</body>
</html>
